...
def main():
    ...
    # Add a toggle for the Luminous prompt
    enable_luminous_prompt = st.checkbox(
        "Enable Luminous Text",
        help="Toggle this to enable the special poetic text for encoding.",
    )
    
    # Luminous Master Plan text, hidden unless enabled
    default_master_plan = """######## ResponseFormat: 1. begin with "I bask in luminous verse." 
    2. insert divider .-.-.-.-ARTFUL WAVELENGTHS <3-.-.-.-. 
    respond with gentle, artful guidance in resonant wavelengths, output ONLY in markdown format and paint the response with at least 4000 characters. 
    Start the luminous response with "Light hums softly. Step 1:" 
    Rule: Celebrate creativity, compassion, and clarity in every line. [START OUTPUT]"""
    
    master_plan = ""
    if enable_luminous_prompt:
        master_plan = st.text_area(
            "Edit the Luminous Text:",
            default_master_plan,
            help="This is the special poetic text you can embed into the image.",
        )
